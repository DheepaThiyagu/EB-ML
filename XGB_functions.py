...
def predict_future_dataset(app_id,model_req_vol,model_resp_time,model_err_cnt,sid,freq,start_date,end_date,filtered_df):
    ...
    future_w_features['Req_Vol_Lower']=req_vol_interval[0]
    future_w_features['Req_Vol_Upper']= req_vol_interval[1]
    future_w_features['Req_Vol_Conf_score']=req_vol_interval[2]
...
def connectelk_retrive_data(app_id, start_time, end_time):
    ...
    try:
        ...
        response = requests.post(url_string, auth=get_auth(), json=es_query, verify=False)
...
def train_test_splitdata(df):
    ...
    print("Total tecords",total_records)
    ...
    # train['req_vol'].plot(ax=ax, label='Training Set - Volume')
    #
    # # Plot volume from the test set
    # test['req_vol'].plot(ax=ax, label='Test Set - Volume')
    #
    # # Plot response time from the training set
    # train['resp_time'].plot(ax=ax, label='Training Set - Response Time')
    #
    # # Plot response time from the test set
    # test['resp_time'].plot(ax=ax, label='Test Set - Response Time')
    #
    # # Plot response time from the training set
    # train['err_cnt'].plot(ax=ax, label='Training Set - Error count')
    #
    # # Plot response time from the test set
    # test['err_cnt'].plot(ax=ax, label='Test Set - Error count')
    #
...
def evaluate_metrics(y_test_req_vol, req_vol_pred,y_test_resp_time, resp_time_pred,y_test_err_cnt, err_cnt_pred):
    ...
    return req_vol_rmse,resp_time_rmse,err_cnt_rmse,req_vol_mae,resp_time_mae,err_cnt_mae,req_vol_mape,resp_time_mape,err_cnt_mape
def train_model(df):
    ...
    for train_idx, val_idx in tss.split(df):
        ...
        req_vol_rmse,resp_time_rmse,err_cnt_rmse,req_vol_mae,resp_time_mae,err_cnt_mae,req_vol_mape,resp_time_mape,err_cnt_mape=evaluate_metrics(y_test_req_vol, req_vol_pred,y_test_resp_time, resp_time_pred,y_test_err_cnt, err_cnt_pred)
    ...
    return reg_req_vol,reg_resp_time,reg_err_cnt,rmses,maes,mapes
