...
def train_volume_model_with_grid_search(df):
    try:
        ...
        for train_idx, val_idx in tss.split(df):
            ...
            req_vol_rmse, req_vol_mae, req_vol_mape, req_vol_mase=evaluate_metrics_req_vol(y_test_req_vol, req_vol_pred)
        ...
        return reg_req_vol, rmses, maes, mapes, mases
...
def train_response_time_model_with_grid_search(df):
    try:
        tss = TimeSeriesSplit(n_splits=2)
        df = df.sort_index()
        ...
        for train_idx, val_idx in tss.split(df):
            ...
            resp_time_rmse, resp_time_mae, resp_time_mape, resp_time_mase=evaluate_metrics_resp_time(y_test_resp_time, resp_time_pred)
            rmses.append((resp_time_rmse,))
            maes.append((resp_time_mae,))
            mapes.append((resp_time_mape,))
            mases.append((resp_time_mase,))

        return reg_resp_time, rmses, maes, mapes, mases
...
def train_error_count_model_with_grid_search(df):
    try:
        tss = TimeSeriesSplit(n_splits=2)
        df = df.sort_index()
        ...
        for train_idx, val_idx in tss.split(df):
            ...
            err_cnt_rmse, err_cnt_mae,err_cnt_mape, err_cnt_mase = evaluate_metrics_err_cnt(y_test_err_cnt, err_cnt_pred)
        ...
        return reg_err_cnt, rmses, maes, mapes, mases
...
def predict_future_dataset(app_id,sid,freq,start_date,end_date):
    # req_vol_preds = []
    # resp_time_preds = []
    # err_cnt_preds=[]
    ...
    try:
        if os.path.exists(model_req_vol):
            ...
            model_req_vol=None
            print(f"Warning: Request volume model not found at {model_req_vol}")

        if os.path.exists(model_err_cnt):
            pred_err.load_model(model_err_cnt)
        else:
            model_err_cnt=None
            print(f"Warning: Error count model not found at {model_err_cnt}")

        if os.path.exists(model_resp_time):
            pred_resp.load_model(model_resp_time)
        else:
            model_resp_time=None
            print(f"Warning: Response time model not found at {model_resp_time}")
...
def train_and_predict_for_service(filtered_df, sid, model_type):
    ...
    if len(filtered_df) <4:
        ...
        if model_type == 'vol':
            request_volume_model = train_request_volume_model(filtered_df, sid)
        ...
        if model_type == 'resp':
            response_time_model = train_response_time_model(filtered_df, sid)
            response_time_model.save_model(os.path.join(project_path, models_path, f'{sid}_{model_type}.json'))
            print(f"Model saved for {sid} ({model_type})")
            return response_time_model
        if model_type == 'err':
            error_count_model = train_error_count_model(filtered_df, sid)
...
def train():
    ...
    df_corr_vol_err = df_corr_vol_err['sid'].tolist()[:2]
    df_corr_vol_resp = df_corr_vol_resp['sid'].tolist()[:2]

    all_forecasts = []

    def process_service(sid):
        # sid = row['sid']
        df_service = df[df['sid'] == sid].sort_values(by='record_time')

        df_service['record_time'] = pd.to_datetime(df_service['record_time'], unit='ms')
        save_csv(f'{sid}_train.csv', df_service)
    ...
    with ThreadPoolExecutor() as executor:
        executor.map(process_service, df_all_services)
