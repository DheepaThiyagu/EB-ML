...
# # Group by app_id and sid, and calculate the minimum total_req_count for each group
# min_vol_for_eb = filtered_eb.groupby(['app_id', 'sid'])['total_req_count'].min().reset_index()
# min_vol_for_resp = filtered_resp.groupby(['app_id', 'sid'])['total_req_count'].min().reset_index()
...
print(min_vol_and_time_for_eb)

min_vol_and_time_for_resp = filtered_resp.groupby(['app_id', 'sid']).agg({
    'total_req_count': 'min',
    'record_time': ['min', 'max']
}).reset_index()
print(min_vol_and_time_for_resp)
# # Merge the two DataFrames on app_id and sid
# result_df = pd.merge(min_vol_for_eb, min_vol_for_resp, on=['app_id', 'sid'], how='outer', suffixes=('_for_eb_breach', '_for_response_breach'))
#
# # Save the result to a CSV file
# result_csv_path = os.path.join(project_path,training_path,'min_vol_for_breaches.csv')
...
# min_vol_and_time_for_eb['day_of_week_for_eb_breach'] = min_vol_and_time_for_eb['start_time_for_eb_breach'].dt.day_name()
# min_vol_and_time_for_resp['day_of_week_for_response_breach'] = min_vol_and_time_for_resp['start_time_for_response_breach'].dt.day_name()
...
# Save the result to a CSV file
result_csv_path = os.path.join(project_path, training_path,'date_min_vol_and_time_for_breaches.csv')
result_df.to_csv(result_csv_path, index=False)
